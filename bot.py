...
# Пока каждое блюдо имеет одинаковое базовое время приготовления.
# Меняйте значения здесь по мере накопления реальных данных.
PREP_MINUTES_BY_DISH: dict[str, int] = {
    name: 5 for name in MENU_PRICE_MAP
}
